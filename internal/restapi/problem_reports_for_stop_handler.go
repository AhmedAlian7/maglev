...
func (api *RestAPI) problemReportsForStopHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Warn("problem reports for stop failed: invalid stopID format",
			slog.String("stopID", compositeID),
			slog.Any("error", err))
		api.sendError(w, r, http.StatusBadRequest, "invalid stopID format")
		return
	}
	...
	if api.GtfsManager == nil || api.GtfsManager.GtfsDB == nil || api.GtfsManager.GtfsDB.Queries == nil {
		logger.Error("problem reports for stop failed: GTFS DB not initialized")
