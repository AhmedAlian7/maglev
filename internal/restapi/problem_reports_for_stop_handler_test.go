...
func TestProblemReportsForStop_InvalidID(t *testing.T) {
	api := createTestApi(t)
	defer api.Shutdown()

	nullURL := "/api/where/problem-reports-for-stop/.json?key=TEST"
	nullResp, nullModel := serveApiAndRetrieveEndpoint(t, api, nullURL)

	assert.Equal(t, http.StatusBadRequest, nullResp.StatusCode)
